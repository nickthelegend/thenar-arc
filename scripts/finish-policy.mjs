/** Fill the last slot of a task, mint its policy, and license it. */
...
function makeRun(deviationMm,secs,ease,seed){
  const hz=20,n=hz*secs,from=[0.3,0.2],goal=[0.17,-0.24],samples=[];
  for(let i=0;i<=n;i++){const u=i/n;const p=u<ease?(u/ease)**2*ease:Math.min(1,ease+(u-ease)/(1-ease)*(1-ease));
    samples.push({t:+(i/hz).toFixed(3),q:[0.1*p,0.5*p,0.9*p,0,1.2*p,0],grip:u>0.05&&u<0.95?6:42,
      object:[from[0]+(goal[0]-from[0])*p+seed*1e-9,from[1]+(goal[1]-from[1])*p,Math.sin(Math.PI*u)*0.18]});}
  samples[samples.length-1].object=[goal[0]+deviationMm/1000,goal[1],0];
...
console.log(`\ntask #${TASK} ${t.slotsFilled}/${t.slotsTotal}\n`);
let seed=100;
while (t.slotsFilled < t.slotsTotal) {
  const op=privateKeyToAccount(generatePrivateKey());
  // Monad reserves value + gas_limit x price; fund well clear of the floor.
  const f=await wallet.sendTransaction({to:op.address,value:parseEther("4")});
  await pub.waitForTransactionReceipt({hash:f});
  check(await awaitFunds(op.address,parseEther("4")),"operator funded",op.address.slice(0,10));
  await new Promise(r=>setTimeout(r,2000));
  const res=await fetch(`${BASE}/api/verify`,{method:"POST",headers:{"content-type":"application/json"},
    body:JSON.stringify({taskId:TASK,contributor:op.address,...makeRun(6.1,88,0.36,seed++)})});
  const v=await res.json();
  if(!res.ok){check(false,"verify",v.error);break;}
  const w=createWalletClient({account:op,chain,transport:http()});
  const h=await w.writeContract({address:AXON,abi,functionName:"submitTrajectory",args:[BigInt(TASK),v.trajHash,v.cid,v.score,v.signature]});
...
let h=await wallet.writeContract({address:AXON,abi,functionName:"mintPolicy",args:[BigInt(TASK),FEE]});
...
check(distinct>1,"weights differ between operators",cap[1].map(b=>(Number(b)/100).toFixed(2)+"%").join(", "));
const buyer=privateKeyToAccount(generatePrivateKey());
const fb=await wallet.sendTransaction({to:buyer.address,value:parseEther("3")});
await pub.waitForTransactionReceipt({hash:fb});
check(await awaitFunds(buyer.address,parseEther("3")),"buyer funded");
await new Promise(r=>setTimeout(r,2000));
...
h=await bw.writeContract({address:AXON,abi,functionName:"licensePolicy",args:[BigInt(pid)],value:FEE});
